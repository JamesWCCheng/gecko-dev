...
public:
  ...
  GMPCDMProxy():mKeys(nullptr){}
  void Set()
  {
    widget::sdk::SurfaceView::LocalRef surfaceview;
    auto rvSurfaceView = widget::sdk::SurfaceView::New(widget::GeckoAppShell::GetContext(), &surfaceview);
    PG(rvSurfaceView);
    mSurfaceView = surfaceview;
    //widget::GeckoAppShell::CreateSurfaceView();
  }
  widget::sdk::SurfaceView::GlobalRef GetSurfaceView()
  {
    PG0();
    auto surfaceview = widget::GeckoAppShell::GetSurfaceView();
    auto local = widget::sdk::SurfaceView::LocalRef::Adopt(surfaceview.Forget());
    mSurfaceView = local;
    return mSurfaceView;
  }
  NS_INLINE_DECL_THREADSAFE_REFCOUNTING(GMPCDMProxy)

  typedef MozPromise<DecryptResult, DecryptResult, /* IsExclusive = */ true> DecryptPromise;

  // Main thread only.
  GMPCDMProxy(dom::MediaKeys* aKeys, const nsAString& aKeySystem);
